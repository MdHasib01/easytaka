...
// Inside ReviewCenterTab:
const hookStr = `const [activeCategory, setActiveCategory] = useState('Enrichment');`;
const newHookStr = `const { accounts, simulateReview, smm } = useSMM();
...
// Change categories to read actual count
code = code.replace(
  `const categories = [
    { id: 'Enrichment', icon: UserCheck, count: 12 },`,
  `const categories = [
    { id: 'Enrichment', icon: UserCheck, count: pendingEnrichments.length },`
);

// Map through pendingEnrichments
const mapStr = `{[1, 2, 3, 4].map((i) => (`
const mapEnd = `                 ))}
              </div>`
              
const newMap = `{pendingEnrichments.length === 0 && (
...
const startMapIdx = code.indexOf(mapStr);
const endMapIdx = code.indexOf(mapEnd) + mapEnd.length;
code = code.substring(0, startMapIdx) + newMap + code.substring(endMapIdx);


// Add the Review Detail Drawer
const modalStr = `
      <AnimatePresence>
        {selectedReviewItem && (
...
code = code.replace(`</Card>`, `</Card>` + modalStr);
